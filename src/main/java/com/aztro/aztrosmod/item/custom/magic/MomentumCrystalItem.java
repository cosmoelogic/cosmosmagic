...
public class MomentumCrystalItem extends Item {
	...
	public TypedActionResult<ItemStack> use(World world, PlayerEntity playerEntity, Hand hand)
	{
		...
		if (!Boost.isPlayerMoving(playerEntity) && playerEntity.isSneaking()) return new TypedActionResult<>(ActionResult.FAIL, playerEntity.getStackInHand(hand));
		if (!playerEntity.isSneaking()) {
			double[] boostVel = Boost.getBoostVel(playerEntity);
	...
	public static class Boost {
		...
		public static void boostEnemy(PlayerEntity player, Entity entity){
			...
			double[] boostVel = getBoostVel(player);
			...
			if (entity.isOnGround()) boostVel[1] = 10; else boostVel[1] = 4;
			entity.addVelocity(boostVel[0], boostVel[1], boostVel[2]);
